...
parser.add_argument('--attack_temporal', default='False', type=str, help='whether attack on temporal wise')
...
def main():
    ...
    log_dir = 'results/'+'%s-results/'% (args.dataset)+'%s'%(args.model) + "/"

    model_dir = 'results/'+'%s-checkpoints/'% (args.dataset)+'%s'%(args.model) + "/"
    if not os.path.exists(log_dir):
        os.makedirs(log_dir)
    
    logger = get_logger(os.path.join(log_dir, '%s.log'%(args.identifier+args.suffix)))
    print(os.path.join(log_dir, '%s.log'%(args.identifier+args.suffix)))
    ...
    for atk_config in config:
        ...
        if args.attack.lower() == 'fgsm':
            atk = attack.FGSM(atkmodel, forward_function=ff, eps=args.eps / 255, T=args.time, args=args)
        elif args.attack.lower() == 'pgd':
            atk = attack.PGD(atkmodel, forward_function=ff, eps=args.eps / 255, alpha=args.alpha / 255, steps=args.steps, T=args.time,args=args)
        elif args.attack.lower() == 'bim':
            atk = attack.BIM(atkmodel, forward_function=ff, eps=args.eps / 255, alpha=args.alpha / 255, steps=args.steps, T=args.time,args=args)
        ...
        if atk is not None:
            acc, grad_norm_sum = val(model, test_loader, device, args.time, dvs, znorm,atk,args=args)
            logger.info('%s'%(args.attack)+'-acc:{:.3f}'.format(acc)+'-TSV:{:.3f}'.format(grad_norm_sum))
        else:
            acc,_ = val(model, test_loader, device, args.time, dvs, znorm,atk,args=args)
